...
Langkah:
1. Verifikasi `config/env.md` terisi dan `explore/module-map.md` ada (jika belum, sarankan `/explore` dulu).
2. Cari folder skenario yang cocok dengan "$1" di `scenario/` (boleh fuzzy match, mis. "ftl auto stuffing" → `oms012-order-ftl-auto-stuffing`). Jika tidak ada dokumen skenario → tawarkan smoke test eksploratif saja dan minta konfirmasi.
