...
- Pola kode: fungsi `async (page) => {...}` yang mengembalikan objek hasil, bukan melempar error mentah. Contoh kerangka:
  ...
  async (page) => {
    ...
    try {
      ...
      await page.getByRole('heading', { name: 'Daftar Order' }).waitFor({ state: 'visible' });
    } catch (e) {
      r.status = 'failed';
