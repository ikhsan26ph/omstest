...
| REQ-065a | Penentuan Mode Penugasan | Berlaku untuk **FCL maupun LCL**. Mode Penugasan (`Tugaskan Ke Pengurus` atau `Tugaskan Ke Sopir`) ditentukan pada saat **pertama kali Tambah Penugasan** dan berlaku tetap untuk penugasan tersebut, termasuk saat Penugasan Sopir Bongkar (REQ-064). | Nilai tidak berubah setelah penugasan awal dibuat; ditampilkan read-only pada form Penugasan Sopir Bongkar (field `Mode Penugasan`). | Penugasan awal dibuat dengan mode tertentu → form Penugasan Sopir Bongkar pada order FCL/LCL yang sama menampilkan Mode Penugasan yang sama (read-only) dan mengikuti aturan metode pengisian mode tersebut (REQ-065b/REQ-065c). |
| REQ-065b | Metode pengisian saat Mode "Tugaskan Ke Pengurus" | Berlaku untuk **FCL maupun LCL**. Jika Mode Penugasan = **Tugaskan Ke Pengurus**, metode pengisian Armada Bongkar & Sopir Bongkar dapat memakai **Pilih Dari Master** maupun **Isi Data Manual**. | Kedua opsi radio (Pilih Dari Master, Isi Data Manual) enabled/dapat dipilih. | Penugasan dengan Mode "Tugaskan Ke Pengurus" (FCL/LCL) → pada form Penugasan Sopir Bongkar kedua opsi radio dapat dipilih dan disimpan. |
| REQ-065c | Metode pengisian saat Mode "Tugaskan Ke Sopir" | Berlaku untuk **FCL maupun LCL**. Jika Mode Penugasan = **Tugaskan Ke Sopir**, metode pengisian Armada Bongkar & Sopir Bongkar hanya **Pilih Dari Master**; opsi **Isi Data Manual** dinonaktifkan. | Opsi radio "Isi Data Manual" disabled/tidak dapat dipilih. | Penugasan dengan Mode "Tugaskan Ke Sopir" (FCL/LCL) → pada form Penugasan Sopir Bongkar opsi radio "Isi Data Manual" tampak disabled dan hanya "Pilih Dari Master" yang dapat dipilih & disimpan. |
...
- Radio `Isi Data Manual` pada Sopir Bongkar dirender lebih redup (disabled) — **dikonfirmasi**: order contoh ini punya `Mode Penugasan = Tugaskan ke Sopir`, sehingga sesuai REQ-065c opsi Isi Data Manual memang dinonaktifkan (tidak bertentangan dengan REQ-065, lihat REQ-065a s.d. REQ-065c dan D-12).
- Mockup layar ini memakai contoh order **FCL**, tapi REQ-065a s.d. REQ-065c berlaku IDENTIK untuk halaman Penugasan Sopir Bongkar pada order **LCL** (dikonfirmasi user/BA, lihat D-12 Update 2) — tidak ada mockup terpisah untuk varian LCL, diasumsikan strukturnya sama (hanya `Ringkasan order`/`Jenis Pengiriman` yang berbeda nilai).
...
| `140.png` | REQ-064, REQ-065, REQ-065a, REQ-065b, REQ-065c (Mode Penugasan), REQ-066 (tombol Batal), REQ-067 (kondisi metode) | Field `Tanggal Permintaan Bongkar` di luar REQ-064 |
...
| D-12 | Field `Metode Penugasan` pada Detail Penugasan | Layar 146/147/148 menampilkan `Metode Penugasan : Tugaskan ke Sopir` yang tidak tercantum pada REQ-070. | Dicatat sebagai field read-only tambahan pada Informasi Penugasan; diverifikasi keberadaannya. **Update:** enum ini punya 2 nilai — `Tugaskan Ke Pengurus` dan `Tugaskan Ke Sopir` — ditentukan saat pertama kali Tambah Penugasan dan tetap berlaku untuk penugasan tsb (REQ-065a). Nilai ini juga mengendalikan metode pengisian Armada/Sopir Bongkar: `Tugaskan Ke Pengurus` → Pilih Dari Master & Isi Data Manual tersedia (REQ-065b); `Tugaskan Ke Sopir` → hanya Pilih Dari Master (REQ-065c). **Update 2 (2026-09-19):** dikonfirmasi eksplisit oleh user/BA bahwa REQ-065a s.d. REQ-065c **berlaku sama untuk order LCL**, bukan cuma FCL — konsisten dengan cakupan section I ("Isi Data Tracking — Ketentuan Khusus FCL / LCL") yang memang mencakup keduanya. Belum ada skenario tertulis (`scenarios.json`/`.feature`) yang menguji jalur LCL untuk area ini secara langsung — seluruh TC-P-056/057, TC-N-042/043/044/045/047/061, TC-E-033/034/035, TC-S-020 memakai precondition order FCL. Ditandai sebagai coverage gap yang diketahui (bukan bug), lihat catatan di `coverage.md`. | Aturan bisnis dikonfirmasi user/BA (bukan lagi asumsi); dituangkan sebagai REQ-065a s.d. REQ-065c, berlaku FCL/LCL. |
