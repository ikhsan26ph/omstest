...
## Addendum 2 — Revisi spesifikasi Step 1 (2026-09-19)

Spesifikasi produk untuk wizard Buat Order Step 1 berubah (ground truth live 2026-09-19, di luar topik Auto Stuffing): dropdown `Tipe Pengiriman` **dihapus** (jadi teks read-only auto-derive), label baris Data Pengirim/Data Penerima berganti dari `Pick Up n`/`Drop Off n` menjadi `Muat n`/`Bongkar n`, tombol tambah baris berganti nama jadi `Tambah Lokasi Muat`/`Tambah Lokasi Bongkar` dan **SELALU tampil** (bukan kondisional), serta minimum baris turun dari asumsi 2 menjadi **1** tanpa mekanisme tolak eksplisit (ikon hapus otomatis hilang saat tersisa 1 baris). Dampak terhadap dokumen coverage ini:

- **Rekomendasi #2 (baris `#2 Minimum alamat Multidrop/Multipoint` pada tabel Addendum 1 di atas) SEKARANG TIDAK RELEVAN dengan bentuk aslinya.** `OMS014-NEG-033`, `OMS014-NEG-043`, `OMS014-NEG-044` semula menguji "hapus alamat ditolak di bawah minimum 2 (tombol Hapus `disabled`)" — premis ini salah per ground truth baru. Ketiga scenario **direvisi isinya** (bukan dihapus, ID tetap sama) agar menguji perilaku yang benar: menghapus alamat hingga tersisa 1 baris membuat label bernomor hilang, ikon hapus otomatis TIDAK DIRENDER (bukan `disabled`), dan `Tipe Pengiriman` auto-revert ke `Normal`. REQ-030/031/032 tetap **Covered** dengan scenario yang sama (isi berbeda).
- Baris RTM `REQ-029..032` (lihat § Requirements Traceability Matrix di bawah) yang menyebut "Pick Up n"/"Drop Off n" pada deskripsi Data Pengirim/Data Penerima kini merujuk `Muat n`/`Bongkar n`; deskripsi Data Barang/Step 2 (mis. `POS-012..016`) TIDAK berubah karena terminologi Pick Up/Drop Off di Data Barang tetap dipakai (lihat `.analysis.md` ground truth poin 8).
- `AC-001.2` (dulu "Dropdown Tipe Pengiriman memuat tepat 4 opsi") direvisi total di `.analysis.md` menjadi deskripsi auto-derive; scenario `OMS014-POS-004` yang mem-verifikasi AC ini ditulis ulang untuk menguji ketiadaan dropdown + auto-update teks, bukan lagi menghitung opsi dropdown.
- 22 scenario terdampak total (grep ulang `Pilih Tipe Pengiriman|Tambah Baris Input|pickup-block|dropoff-block` pada `.feature`/`.json`): `POS-004, POS-006, POS-011, POS-054(diperiksa, TIDAK terdampak — dropdown itu milik Filter Daftar Order, bukan wizard), POS-068, POS-072, POS-074, NEG-001, NEG-002, NEG-003, NEG-006, NEG-007, NEG-033, NEG-043, NEG-044, EDG-009, EDG-011, EDG-020, EDG-033, EDG-035, STR-002, STR-006`. Jumlah scenario total tetap **173** (tidak ada penambahan/penghapusan ID).
- **[UPDATE 2026-09-19, sore]** Kedua item di bawah ini SEBELUMNYA ditandai "belum disentuh/butuh verifikasi lanjutan" — SEKARANG sudah diverifikasi live oleh user dan didokumentasikan sebagai fakta:
  - **Modal rincian alamat multipickup/multidrop** (S-09, dibuka dari Step 3 via link `Lihat Detail` — scenario `OMS014-POS-036`, `OMS014-POS-037`): ternyata berubah LEBIH JAUH dari sekadar label Pick Up/Drop Off. **Judul modal sekarang `Data Pengirim`** (Multipickup, terkonfirmasi via order test 2 Muat) / **`Data Penerima`** (Multidrop, inferensi simetri) — bukan lagi "Detail Multipickup"/"Detail Multidrop". **Isi modal berformat numbered list** (`1.`/`2.`/dst: nama Drop Point, nama Pengirim/Penerima, alamat lengkap 2×, `PIC: <nama> (<no WA>)`), bukan lagi heading "Pick Up n"/"Drop Off n". `OMS014-POS-036`/`OMS014-POS-037` di `.feature`/`.scenarios.json` sudah direvisi menyesuaikan (nama dialog + assertion konten). Lihat S-09 dan ASM-031 di `.analysis.md`/`.ui-inventory.md`.
  - **Pemicu tombol `Simpan ke Draf`** (`OMS014-EDG-033`, ASM-028): terkonfirmasi **unconditionally visible sejak render pertama Step 1**, tidak bergantung jenis order dipilih atau field terisi. `OMS014-EDG-033` di `.feature`/`.scenarios.json` sudah direvisi (dihapus assertion "tidak menampilkan Simpan ke Draf sebelum jenis order dipilih").
  - **Ikon hapus baris `Muat n`/`Bongkar n` di Edit Order** (S-14): terkonfirmasi via inspeksi DOM — TIDAK PERNAH ada pada baris manapun (dulu ditandai "confidence sedang", sekarang fakta pasti). Tidak ada scenario JSON/feature yang perlu direvisi untuk poin ini (belum ada scenario yang menguji keberadaan ikon hapus di Edit Order secara eksplisit).
...
- **Signifikan / disarankan ditambah** (gap fungsional nyata, bukan sekadar tagging): **REQ-025, REQ-027** (tidak ada test bypass/deep-link untuk Step 4 & Detail Order — padahal Step 2 sudah punya NEG-020/NEG-021 untuk requirement sejenis REQ-004). ~~REQ-031, REQ-032 (tidak ada test "hapus alamat sampai kurang dari minimum ditolak"...)~~ **sudah tidak relevan, lihat Addendum 2 (2026-09-19)** — premis minimum=2/tolak tidak berlaku lagi, NEG-043/NEG-044 sudah ada dan direvisi isinya.
...
2. **[SUDAH TIDAK RELEVAN, direvisi 2026-09-19 — lihat Addendum 2]** ~~Tambahkan scenario negative "hapus alamat sampai kurang dari minimum ditolak" untuk **Multidrop** dan **Multipoint**, setara `OMS014-NEG-033`~~. Premis "ditolak di bawah minimum 2" tidak lagi berlaku (minimum turun ke 1, tanpa mekanisme tolak). `NEG-033`/`NEG-043`/`NEG-044` sudah ada dan sudah direvisi isinya untuk menguji perilaku baru (ikon hapus hilang otomatis saat tersisa 1 baris + auto-revert Tipe Pengiriman ke Normal) — tidak perlu scenario tambahan lagi.
